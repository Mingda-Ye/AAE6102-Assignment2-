...
%% Plot all figures =======================================================
    ...
    mean_E_error=mean(abs(navSolutions.E - refCoord.E));rmse_E=rms(abs(navSolutions.E - refCoord.E));
    mean_N_error=mean(abs(navSolutions.N - refCoord.N));rmse_N=rms(abs(navSolutions.N - refCoord.N));
    mean_U_error=mean(abs(navSolutions.U - refCoord.U));rmse_U=rms(abs(navSolutions.U - refCoord.U));
    
    fprintf('mean error: E:%f, N:%f, U:%f\n',mean_E_error,mean_N_error,mean_U_error);
    fprintf('rmse: E:%f, N:%f, U:%f\n',rmse_E,rmse_N,rmse_U);
...
if (~isempty(navSolutions))
    ...
    if isnan(settings.truePosition.E) || isnan(settings.truePosition.N) ...
        ...
        ref_llh=[22.3198722, 114.209101777778, 3.0];
        ref_xyz=llh2xyz([ref_llh(1)*pi/180,ref_llh(2)*pi/180,ref_llh(3)]);

        %=== Convert to UTM coordinate system =============================
        utmZone = findUtmZone(ref_llh(1), ref_llh(2));
        
        % Position in ENU
        [refCoord.E, ...
         refCoord.N, ...
         refCoord.U] = cart2utm(ref_xyz(1), ref_xyz(2), ...
                                                ref_xyz(3), ...
                                                utmZone);
