...
%% Acquisition ============================================================
    ...
    if ((settings.skipAcquisition == 0) || ~exist('acqResults', 'var'))
        ...
        fineacqdata = fread(fid,dataAdaptCoeff*samplesPerCode*11,settings.dataType)';
        if (dataAdaptCoeff==2)    
            data1=data(1:2:end);    
            data2=data(2:2:end);    
            data=data1 + 1i .* data2;    
            % fine acquisition
            data1=fineacqdata(1:2:end);
            data2=fineacqdata(2:2:end);
            fineacqdata=data1 + 1i .* data2; 
        end

        %--- Do the acquisition -------------------------------------------
        disp ('   Acquiring satellites...');
        %acqResults = acquisition_gps(data, settings);
        acqResults = acquisition_hd(data, settings,fineacqdata);
...
if settings.MMT==0
    ...
    startTime = now;
    disp (['   Tracking with MMT started at ', datestr(startTime)]);
    [trackResults, channel] = tracking_MMT(fid, channel, settings);
    fclose(fid);
    disp('   Saving MMT Tracking results to file "trackingResults.mat"')
    save('trackingResults', ...
        'trackResults', 'settings', 'acqResults', 'channel');
    disp(['   Tracking with MMT is over (elapsed time ', ...
        datestr(now - startTime, 13), ')'])
end
%% Calculate navigation solutions =========================================
    disp('   Calculating navigation solutions...');

    [navSolutions, eph] = postNavigation_2(trackResults, settings);
